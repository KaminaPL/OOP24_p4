...
public class ImageController {
    ...
    public ResponseEntity<String> adjustBrightness(@RequestBody umcs.image.ImageDTO imageDTO) throws IOException {
    ...
    private BufferedImage changeBrightness(BufferedImage originalImage, int brightness) {
        ...
        for (int y = 0; y < originalImage.getHeight(); y++) {
            for (int x = 0; x < originalImage.getWidth(); x++) {
                ...
                int r = umcs.image.Clamp.clamp(color.getRed() + brightness, 0, 255);
                int g = umcs.image.Clamp.clamp(color.getGreen() + brightness, 0, 255);
                int b = umcs.image.Clamp.clamp(color.getBlue() + brightness, 0, 255);
